 ...
 * installation.
 */
function oss_form_alter(&$form, $form_state, $form_id) {
  // Proces all forms during installation except the Drupal default
  // configuration form
  if (defined('MAINTENANCE_MODE') && MAINTENANCE_MODE == 'install' &&
      $form_id != 'install_configure_form') {
    array_walk_recursive($form, '_oss_remove_form_requirements');
  }
}
 ...
 */
function oss_form_install_configure_form_alter(&$form, $form_state) {
 ...
 */
function oss_install_tasks(&$install_state) {
 ...
 * fully functional.
 */
function oss_fetch_ding_install_tasks(&$install_state) {
 ...
 */
function _oss_remove_form_requirements(&$value, $key) {
